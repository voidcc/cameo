...
def recolorRC(src, dst):
	...
	# extract the source images's channels as one-dimensional arrays.
	b, g, r = cv2.split(src)

	# replace the B channel's values with an average of B and G.
	# arguments (in order): the first source array, a weight applied to
	# the first source array, the second source array, a weight applied
	# to the second source array, a constant added to the result, the
	# destination array.
	cv2.addWeighted(b, 0.5, g, 0.5, 0, b)

	# replace the values in our destination image with the modified channels.
	cv2.merge((b, b, r), dst)
...
def recolorRGV(src, dst):
	...
	# the min() function computes the per-element minimums of the first
	# two arguments and writes them to the third argument.
	cv2.min(b, g, b)
	cv2.min(b, r, b)
...
def recolorCMV(src, dst):
	...
	# the max() function computes the per-element maximums of the first
	# two arguments and writes them to the third argument.
	cv2.max(b, g, b)
	cv2.max(b, r, b)
...
class BGRPortraCurveFilter(BGRCurveFilter):
	...
	def __init__(self, dtype = numpy.uint8):
		BGRCurveFilter.__init__(self,
			vPoints = [(0,0), (23,30), (157,173), (255,255)],
...
class BGRVelviaCurveFilter(BGRCurveFilter):
	...
	def __init__(self, dtype = numpy.uint8):
		BGRCurveFilter.__init__(self,
			vPoints = [(0,0), (128,128), (221,215), (255,255)],
...
class FindEdgesFilter(VConvolutionFilter):
	"""An edge-finding filter with a 1-pixel raduis."""
...
class BlurFilter(VConvolutionFilter):
	"""A blue filter with a 2-pixel radius."""
...
class EmbossFilter(VConvolutionFilter):
	"""An emboss filter with a 1-poxel radius."""
